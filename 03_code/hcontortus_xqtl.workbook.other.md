...
# used pathfind to get sequencing data
while read name id; do
	pf data --rename --filetype fastq -l ./ --type lane --id ${id};
	done < sample_lanes.list

# map reads
while read name lane; do
	~sd21/bash_scripts/run_bwamem_splitter ${name} /nfs/users/nfs_s/sd21/lustre118_link/hc/XQTL/01_REFERENCE/HAEM_V4_final.chr.fa $PWD/${lane}_1.fastq.gz $PWD/${lane}_2.fastq.gz;
	done < sample_lanes.list
